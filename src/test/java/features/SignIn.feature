Feature: Amazon user should signIn/ Login on application or Amazon Login functionality check
  ...
  Scenario: Amazon login signIn functionality checking using valid credential
   # Given user on amazon Sign-In page
    And user enter valid "ammer501@hotmail.com" email address or phone number in username field
    And user click on continue button
    And user enter valid password "amirqa2021" in password Field
    When user click on Sign-In button
    And user should be able to see the valid "Hello, Amirouche" user name
